    ...
    class EBNFVisitor extends base {
        ...
        grammar(ctx) {
            ...
            validateGrammar({
                rules,
                maxLookahead: 1,
                tokenTypes: Object.values(p.terminals),
                grammarName: p.options.name,
            });

			let retval = (p.options.module) ? generateParserModule({
				name: p.options.name,
				rules
			}) : generateParserFactory({
				name: p.options.name,
				rules,
				tokenVocabulary: Object.values(p.terminals)
			});
			return retval;
        }
